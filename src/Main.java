...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int sum = 0;
        int stItem=0;//стоимость выбранного товара
        //boolean doBonus = sum >= MIN_COST_FOR_BONUS;
        System.out.println("Ваша корзина покупок:");
        //Логика: цена за 3 одинаковых товара из распродажных как за две
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0) {
        ...
        //Логика +1 товар если сумма корзины более 1000

       boolean doBonus = sum >= MIN_COST_FOR_BONUS;
       if (doBonus) {
           System.out.println("Бонус!!! Сумма вашей покупки превысила 1000р! Добавляем по 1 шт к каждому выбранному Вами товару!!!");
           for (int i = 0; i < products.length; i++) {
               boolean isOnSale = false;
               for (String saleProduct : productsOnSale) {
                   if (products[i].equals(saleProduct)) {
               ...
               if (counts[i] != 0) {
                    System.out.println("\t" + products[i] + " " + (doBonus ? counts[i] + 1 : counts[i]) + " шт. за " + prices[i]*(isOnSale ? ( counts[i] / 3 * 2 + counts[i] % 3):counts[i])+ " руб.");
               }
           }
       }
